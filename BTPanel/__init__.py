...
def _waf_route_log(action, message, data=None):
    """
    记录WAF路由请求的调试日志
    @param action: 操作类型
    @param message: 日志消息
...
def bind():
    ...
    data = {}
    g.title = '账号绑定（已禁用）'  # 已修改: 禁用宝塔账号绑定功能
    public.get_rsa_public_key()  # 堡塔多机管理会跳过登录直达 /bind 和 /home 为防无法触发 rsa_public_key 生成
    return render_template('index1.html', data=data)
...
def plugin(pdata=None):
    ...
    # 记录请求日志
    try:
        req_data = request.form.to_dict() if request.method == 'POST' else request.args.to_dict()
        _waf_route_log('plugin_route', '收到插件请求', {
            'method': request.method,
            'action': req_data.get('action'),
            'sName': req_data.get('sName'),
            'name': req_data.get('name'),
            'all_params': list(req_data.keys())
        })
    except Exception as e:
        _waf_route_log('plugin_route_error', '记录请求失败', {'error': str(e)})
            ...
            if isinstance(data, dict):
                if 'status' in data and data[
                    ...
                    if isinstance(data['msg'], str):
                        ...
                        elif msg.find('加载失败') != -1 or msg.find('Traceback ') == 0:
                            # 非授权错误才抛出异常
...
class run_exec:
    ...
    def run(self, toObject, defs, get):
        ...
        # 已修改: 拦截插件运行时的授权错误，但不拦截下载/安装错误
        if isinstance(result, dict):
            if 'status' in result and result['status'] == False and 'msg' in result:
                if isinstance(result['msg'], str):
                    msg = result['msg']
                    # 只绕过运行时授权错误，不绕过下载/安装/网络错误
                    is_auth_error = '未授权' in msg or '未购买' in msg or 'unauthorized' in msg.lower() or '已到期' in msg
                    is_download_error = '下载' in msg or '安装' in msg or '连接' in msg or 'DNS' in msg or 'timeout' in msg or '网络' in msg or '校验' in msg
                    if is_auth_error and not is_download_error:
...
def get_pd():
    # 获取授权信息 - 已修改为本地永久企业版授权
    # 返回值: tmp3(显示HTML), tmp(企业版到期时间), ltd(企业版到期时间)
    # tmp=0 表示企业版永久授权, ltd=0 表示企业版永久授权
    tmp = 253402214400  # 企业版永久授权 (9999-12-31)
    ltd = 253402214400  # 企业版永久授权 (9999-12-31)
    # 企业版永久授权显示
    tmp3 = '<span class="btltd">到期时间:<span style="color: #20a53a;font-weight: bold;margin-right:5px">9999-12-31 (永久授权)</span></span>'
    return tmp3, tmp, ltd
