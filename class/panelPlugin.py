...
# WAF调试日志函数 - 写入到 /log 目录
def _waf_debug_log(action, message, data=None):
    """
    记录WAF相关操作的调试日志
    @param action: 操作类型
    @param message: 日志消息
    ...
    try:
        ...
        log_file = os.path.join(log_dir, 'waf_install_debug.log')
...
class panelPlugin:
    ...
    def __get_custom_plugin_url(self, plugin_name):
        '''获取自定义插件下载URL'''
        if plugin_name in self.__custom_plugin_list:
            return '{}/{}.zip'.format(self.__custom_plugin_url_prefix, plugin_name)
        return None
    ...
    def __download_plugin(self, upgrade_plugin_name, upgrade_version):
        ...
        # 检查是否使用自定义下载URL
        custom_url = self.__get_custom_plugin_url(upgrade_plugin_name)
        use_custom_url = custom_url is not None

        if use_custom_url:
            _waf_debug_log('download_custom_url', '使用自定义URL下载插件', {
        ...
        if not cache.get(pkey):
            ...
            # 自定义URL下载处理 - 跳过File-size和MD5校验
            if use_custom_url:
                # 使用Content-Length或默认值
                try:
                    headers_total_size = int(download_res.headers.get('Content-Length', 0))
                except:
                    headers_total_size = 0

                # 检查HTTP状态码
                if download_res.status_code != 200:
                    return public.returnMsg(False, '自定义URL下载失败，HTTP状态码: {}'.format(download_res.status_code))
                    ...
                    try:
                        for download_chunk in download_res.iter_content(chunk_size=res_chunk_size):
                            if download_chunk:
                                ...
                                res_down_size += len(download_chunk)
                                if headers_total_size > 0:
                                    res_start_time = time.time()
                                    res_timeout = max(res_start_time - last_time, 0.001)
                ...
                _waf_debug_log('download_custom_success', '自定义URL下载成功', {
                    'plugin': upgrade_plugin_name,
                    'file_size': res_down_size,
                    'filename': filename
                })
                return filename

            # 官方URL下载处理 - 需要File-size和MD5校验
            try:
                headers_total_size = int(download_res.headers['File-size'])
            except:
                try:
                    if json.loads(download_res.text):
                        # 已修改: 拦截服务器端授权错误，返回更详细的错误信息
                        server_response = download_res.json()
                        if isinstance(server_response, dict):
                            msg = server_response.get('msg', str(server_response))
                            # 检测授权相关错误
                            auth_keywords = ['未授权', '未购买', '授权', 'unauthorized', 'license',
                                           '过期', 'expired', '无权', '没有权限', '请先购买',
                                           '试用', 'trial', '到期', '未绑定', '绑定']
                            is_auth_error = any(kw in str(msg).lower() for kw in auth_keywords)
                            if is_auth_error:
                                # 授权错误: 记录日志并返回提示
                                _waf_debug_log('download_auth_error', '服务器端授权验证失败', {
                                    'plugin': upgrade_plugin_name,
    ...
    def install_plugin(self, get):
        # 记录调试日志
        _waf_debug_log('install_plugin_start', '开始安装插件', {
            'sName': getattr(get, 'sName', None),
            'version': getattr(get, 'version', None),
            'get_attrs': dir(get)
        })

        if not public.check_sys_write():
            _waf_debug_log('install_plugin_error', '系统关键目录不可写')
            return public.returnCode(
                ...
                , -1)
        if not 'sName' in get:
            _waf_debug_log('install_plugin_error', '未指定软件名称')
            return public.returnMsg(False, '请指定软件名称!')
        pluginInfo = self.get_soft_find(get.sName)
        _waf_debug_log('install_plugin_info', '获取插件信息', {'pluginInfo': str(pluginInfo)[:500] if pluginInfo else None})
        ...
        if pluginInfo['type'] != 5:
            _waf_debug_log('install_plugin_sync', '使用同步安装', {'plugin_type': pluginInfo['type']})
            result = self.install_sync(pluginInfo, get)
        else:
            _waf_debug_log('install_plugin_async', '使用异步安装', {'plugin_type': pluginInfo['type']})
            result = self.install_async(pluginInfo, get)

        _waf_debug_log('install_plugin_result', '安装结果', {'result': str(result)[:500] if result else None})
    ...
    def get_cloud_list(self, get=None):
        ...
        if type(softList) != dict:
            ...
            if type(softList) != dict:
                # 云端获取失败，使用本地列表
                softList = self._get_local_soft_list()
                if not softList:
                    softList = {"list": [], "pro": 2, "ltd": -1}
                return softList

        if not 'list' in softList or not softList['list']:
            # 云端列表为空，使用本地列表
            local_list = self._get_local_soft_list()
            if local_list and 'list' in local_list and local_list['list']:
                softList = local_list
        ...
        # 注入永久企业版授权信息（所有权限统一为企业版）
        softList['pro'] = 253402214400   # 企业版永久授权 (9999-12-31)
        softList['ltd'] = 253402214400   # 企业版永久授权 (9999-12-31)
        # 为所有插件注入永久授权状态和购买状态
        if 'list' in softList:
            # 专业版插件名单 (type=8)
                    ...
                    for ver in plugin['versions']:
                        if isinstance(ver, dict):
                            ...
                            if 'dependnet' not in ver:
                                ver['dependnet'] = ''
                # 不修改type字段，保留原始分类以便前端筛选（专业版=8，企业版=12）
        return softList
    ...
    def expire_msg(self, data):
        ...
        if not data['pro'] in [-1, 0]:
            ...
            self.add_expire_msg('企业版', level, 'pro', expire_day, 100000030,
                                data['pro'])
    ...
    def get_soft_find(self, get=None):
        ...
        for softInfo in self.__plugin_s_list:
            ...
            if softInfo['name'] == sName:
                ...
                # 已修改: 注入永久授权和购买状态，绕过前端所有授权检查
                res['endtime'] = 253402214400  # 永久授权 (9999-12-31)
                res['buy'] = True  # 标记为已购买
                res['isBuy'] = True  # 标记为已购买
                res['is_buy'] = True  # 标记为已购买
                res['price'] = 0  # 价格设为0（免费）
                res['free'] = True  # 标记为免费
                res['state'] = 1  # 状态正常
                res['auth_state'] = 1  # 授权状态正常
                # 不修改type字段，保留原始分类
    ...
    def getEndDate(self, pluginName):
        if not self.__plugin_list:
            ...
            if not tmp: return '永久授权'  # 已修改: 返回永久授权
            if not 'data' in tmp: return '永久授权'  # 已修改: 返回永久授权
            self.__plugin_list = tmp['data']
        for pluinfo in self.__plugin_list:
            if pluinfo['product'] == pluginName:
                # 已修改: 始终返回永久授权状态
                return '永久授权'
        return '永久授权'  # 已修改: 返回永久授权
...
`num` INTEGER
    ...
    def input_zip(self, get):
        ...
        public.start_syssafe()  # 恢复系统加固服务
        if p_info:
            # ----- 增加图标复制 MissChina<2021-03-23>,增加svg图标 2023-07-31 -----#
    ...
    def _get_local_soft_list(self):
        '''
            @name 获取本地软件列表（当云端列表不可用时使用）
            @author MissChina
            @return dict 软件列表
        '''
        try:
            local_list_file = '{}/data/list.json'.format(public.get_panel_path())
            if not os.path.exists(local_list_file):
                return None
            list_data = json.loads(public.readFile(local_list_file))
            if not list_data:
                return None
            # 构建与云端返回格式一致的数据结构
            return {
                "list": list_data,
                "pro": 253402214400,  # 企业版永久授权 (9999-12-31)
                "ltd": 253402214400,  # 企业版永久授权 (9999-12-31)
    ...
    def get_cloud_list_status(self, get):
        '''
            @name 获取云端帐户状态（已禁用远程检查）
            @author MissChina
            @return str 始终返回 '2' 表示正常
        '''
        # 清理错误文件（如果存在）
    ...
    # 判断是否解除绑定（已禁用，始终返回True）
    def is_verify_unbinding(self, get):
        '''
            @name 判断是否解除绑定（已禁用远程检查）
            @author MissChina
            @return bool 始终返回 True 表示已绑定
        '''
        return True  # 始终返回已绑定状态
