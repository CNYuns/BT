...
class Base(object):
    ...
    def get_firewall_status(self) -> bool:
        '''
            @name 获取防火墙运行状态
            @author MissChina <2024/3/14 上午 11:27>
            @param
    ...
    def set_ping(self, get) -> dict:
        '''
            @name 设置禁ping
            @author MissChina <2024/3/14 上午 11:31>
    ...
    def get_www_logs_size(self, get) -> Dict[str, Union[str, Any]]:
        '''
            @name 获取网站日志目录的大小
            @author MissChina <2024/3/14 上午 11:37>
            @param
            @return dict{"status":True/False,"msg":"提示信息"}
        '''
    ...
    def check_resolve_crontab(self):
        '''
            @name 检查是否已添加计划任务
            @author MissChina <2024/3/26 下午 5:41>
            @param "data":{"参数名":""} <数据类型> 参数描述
            @return dict{"status":True/False,"msg":"提示信息"}
        '''
    ...
    def remove_resolve_crontab(self):
        '''
            ...
            @author MissChina <2024/3/26 下午 11:37>
            @param "data":{"参数名":""} <数据类型> 参数描述
            @return dict{"status":True/False,"msg":"提示信息"}
        '''
    ...
    def CheckPort(self, port, protocol):
        '''
            @name 端口扫描
            @author MissChina <2024/3/26 下午 6:22>
            @param "data":{"参数名":""} <数据类型> 参数描述
            @return dict{"status":True/False,"msg":"提示信息"}
        '''
        import socket
