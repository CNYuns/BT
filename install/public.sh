...
if [ -f "/www/server/panel/data/node.json" ];then
	NODE_FILE_CHECK=$(cat /www/server/panel/data/node.json 2>/dev/null | grep 125.88.182.172)
...
if [ -f "/www/server/panel/install/d_node.pl" ];then
	...
	FILE_DATE=$(stat /www/server/panel/install/d_node.pl 2>/dev/null | grep Change | awk '{print $2}')
...
get_node_url(){
	...
	do
		...
		if [ "${NODE_STATUS}" = "200" ];then
			if [ "$TIME_TOTAL" -lt 300 ] 2>/dev/null;then
				if [ "$RES" -ge 1500 ] 2>/dev/null;then
			...
			i=$((${i:-0}+1))
			if [ "$TIME_TOTAL" -lt 300 ] 2>/dev/null;then
				if [ "$RES" -ge 2390 ] 2>/dev/null;then
					break
				fi
			fi
		fi
	...
	NODE_URL=$(cat $tmp_file1 2>/dev/null | sort -r -g -t " " -k 1 | head -n 1 | awk '{print $2}')
	if [ -z "$NODE_URL" ];then
		NODE_URL=$(cat $tmp_file2 2>/dev/null | sort -g -t " " -k 1 | head -n 1 | awk '{print $2}')
		if [ -z "$NODE_URL" ];then
	...
	# 保存节点选择结果
	echo $NODE_URL > /www/server/panel/install/d_node.pl
}
...
GetSysInfo(){
	...
	CMAKE_VER=$(cmake --version 2>/dev/null | grep version | awk '{print $3}')
...
if [ -z "$NODE_URL" ];then
	EN_CHECK=$(cat /www/server/panel/config/config.json 2>/dev/null | grep English)
	if [ -z "${EN_CHECK}" ];then
		echo '正在选择下载节点...'
	else
