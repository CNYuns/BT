...
class Sql():
    ...
    __OPT_FIELD = "*"  # field条件
    __OPT_PARAM = ()  # where值
    __LOCK = '/dev/shm/sqlite_lock2.pl'

    def __init__(self):
        self.__DB_FILE = '/www/server/btwaf/totla_db/totla_db.db'
        ...
        try:
            if self.__DB_CONN == None:
                ...
                if os.path.exists(self.__DB_FILE):
                    #获取文件的uid
                    uid = os.stat(self.__DB_FILE).st_uid
                    #判断是否为www用户
                    if uid ==0:
                        os.system("chown www:www {}".format(self.__DB_FILE))
    ...
    def dbfile(self, name):
        self.__DB_FILE = '/www/server/btwaf/totla_db/' + name + '.db'
    ...
    def __get_columns(self):
        if self.__OPT_FIELD == '*':
            ...
            cols = []
            for col in tmp_cols:
                if len(col) > 2: cols.append('`' + str(col[1]) + '`')
