var bt_waf_tools = {
        ...
        ReaderTable.prototype = {
            ...
            $load: function () {
                ...
                if (this.config.tootls) {
                    ...
                    if ($(_that.config.el + '.divtable').length === 0) $(_that.config.el).append('<div class="divtable mtb10" style="heigth:' + (this.config.height || 'auto') + '"></div>');
                }
                this.$reader_content();
                if (_that.config.url !== undefined) {
                    ...
                    if(typeof this.config.data == 'object'){
                        this.$reader_content(this.config.data.data,this.config.data.total);
                    }else{
            ...
            */
            $refresh_table_list: function (load, callback) {
                ...
                if (load) loadT = bt_waf_tools.load('获取列表数据');
                ...
                if (!this.init) {
                    ...
                    if (this.config.page) {
                        $(this.config.el + ' .page').replaceWith(this.$reader_page(this.config.page, page));
								...
								function hover_tips(){
									if(data.length){
										// 循环bt.config.column，如果有hover_tips属性，则给对应的列头添加tips
										config.column.forEach(function(item,index){
											if(item.hover_tips){
												$('.'+item.elName).on({
													mouseenter:function(e){
														var that = this
															tips =layer.tips(item.hover_tips, that, {
																tips: [3, '#555'],skin:'hover_tips',time:0,auto:'200px',
															});
													},
													mouseleave:function(){
															layer.close(tips);
            ...
            $reader_column_type: function (item, rows) {
                ...
                switch (item.type) {
                    ...
                    case 'check':  //单个单选框
                        ...
                        config = ['<label><i class="cust—checkbox '+item.active(rows) +' cursor-pointer ' + className + '"></i><input type="checkbox" class="cust—checkbox-input"></label>', event_list];
                         break;
                    case 'checkSpan':  //单选加标签
                        if (!event_list[className] && item.event) event_list[className] = {
                            event: item.event,
                            type: 'rows',
                            check:'checkSpan'
                        };
                        config = ['<label><i class="cust—checkbox '+item.active(rows) +' cursor-pointer ' + className + '"></i><input type="checkbox" class="cust—checkbox-input"><span class="tipsval '+(item.label[1](rows) > 0?"grade_1":"")+'" title="'+item.label[0]+':'+item.label[1](rows)+'">'+item.label[1](rows)+'</span></label>', event_list];
                         break;
                    case 'password':
                        var _copy = '',
                            _eye_open = '',
                        ...
                        case 'group':
                            $.each(item.list, function (index, items) {
                                ...
                                if (items.type == 'division') {
                                    ...
                                    if (!items.group) {
                                        template += '<button type="button" title="' + (items.tips || items.title) + '" class="btn ' + (items.active ? 'btn-success' : 'btn-default') + ' ' + _btn + ' btn-sm mr5">' + (items.icon ? '<span class="glyphicon glyphicon-' + items.icon + ' mr5"></span>' : '') + '<span>' + items.title + '</span></button>';
                                    } else {
            ...
            */
             ...
             * @param {}
            */
            $custom_page: function (total, number) {
            ...
            $http: function (success) {
                ...
                bt_waf_tools.send({
                    ...
                    if (typeof config.dataFilter != "undefined") {
                        ...
                        if (void 0 == res.data && res.msg.data) {
                            success && success({
                                data: res.msg.data,
                                page: res.msg.page
                            })
    ...
    open: function (config) {
        ...
        if (typeof _config.content == "object") {
            ...
            form = bt_waf_tools.form(param);
            _config.success = function (layero, indexs) {
                form.$event_bind();
                if (typeof _config.success != "undefined") {
                    if (typeof param.success != "undefined") param.success(layero, indexs);
                }
            }
    ...
    msg: function (param1, param2) {
        ...
        // if(param1.msg == {}) return false
        if(JSON.stringify(msg) == '{}') return false
    ...
    send: function (param1, param2, param3, param4, param5, param6) {
        ...
        $.ajax({
                ...
                if (res.status === false && (res.hasOwnProperty('msg') || res.hasOwnProperty('error_msg'))) {
                    if (error) {
                      error(res)
                        ...
                        bt_waf_tools.msg({ status: res.status, msg: !res.hasOwnProperty('msg') ? res.error_msg : res.msg });
                    }
                    return false;
                  }
                if (params.tips) {
            ...
            error: function (er) {
                ...
                layer.msg('服务响应状态码：' + (er.status == 0 ? '--' : er.status) + '</br>错误信息：' + er.statusText + '</br>接口信息：' + params.url + '</br>接口参数：' + JSON.stringify(params.data), {
                    icon: 2,
                    time: 0,
                    closeBtn: 2
                });
