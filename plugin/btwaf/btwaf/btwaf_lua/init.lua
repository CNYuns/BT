...
#-------------------------------------------------------------------
# Author: lkq@bt.cn 
# Version 9.8.0 2025-09-09
# Time:2025-09-09 09:09:09
#-------------------------------------------------------------------
]]--
BTWAF_OBJS = {}
BTWAF_OBJS.version ="2025-09-09 9.8.0"
BTWAF_RULES = {}
BTWAF_CMS_OBJS={}
...
-- 兼容低版本的Nginx
local btwaf_config=Public.read_file_body("/www/server/panel/vhost/nginx/btwaf.conf")
BTWAF_RULES.header_status=false
if btwaf_config and  string.find(btwaf_config,'body_filter_by_lua_file') then
    if  string.find(btwaf_config,'#body_filter_by_lua_file') then
        BTWAF_RULES.header_status=false
    else 
        BTWAF_RULES.header_status=true
    end
end
