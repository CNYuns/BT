...
local function get_cookie_table_rule(text_cookie)
    if type(text_cookie) ~= "string" then
        log(ERR, format("expect text_cookie to be \"string\" but found %s",
                type(text_cookie)))
        return {}
    ...
    while j <= len do
        ...
        elseif state == EXPECT_VALUE then
            if byte(text_cookie, j) == SEMICOLON then
                value = sub(text_cookie, i, j - 1)
                if has_special_chars(value) then
                    if cookie_table[key] ~= nil then
                        local random_suffix = "_" .. tostring(ngx.now()*1000):sub(-6)
...
function _M.get_all_rule(self)
    if not self._cookie then
        return nil, "no cookie found in the current request"
    end

    if self.cookie_table_rule == nil then
        self.cookie_table_rule = get_cookie_table_rule(self._cookie)
    end

    return self.cookie_table_rule
end
