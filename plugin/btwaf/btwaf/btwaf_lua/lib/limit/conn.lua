...
function _M.new(dict_name, max, burst, default_conn_delay,timeout)
...
function _M.incoming(self, key, commit)
    ...
    local conn, err
    if commit then
        --conn, err = dict:incr(key, 1, 0,self.timeout)
        if dict:get(key) then 
             conn, err= dict:incr(key, 1)
        else 
            conn, err = dict:set(key,1,self.timeout)
            conn=1
        end 
        if not conn then
            return nil, err
...
function _M.leaving(self, key, req_latency,count)
    assert(key)
    local dict = self.dict

    local conn, err = dict:incr(key, -count)
    if not conn then
        return nil, err
    end
