...
function post.post()
	...
	if not content_type then 
		ngx.req.read_body()
	     ...
	     if body_info then 
	    	BTWAF_OBJS.request_check.args_urlencoded({body_info})
	    end
		return false 
	end 
	...
	if content_type and ngx.re.find(content_type, 'application/xml',"oij") then 
		ngx.req.read_body()
	    local body_info=ngx.req.get_body_data()
	    if body_info then 
			BTWAF_OBJS.request_check.args_urlencoded({body_info})
	...
	if content_type and  ngx.re.find(content_type, 'application/json',"oij") and ngx.req.get_headers(20000)["content-length"] and tonumber(ngx.req.get_headers(20000)["content-length"]) ~= 0 then
		...
		if  ok and  type(request_args_json)=='table' then 
		    request_args_json=Public.process_json_args_ret(request_args_json)
	...
	if Config['password']~=nil and Config['password'] and  request_args['password'] and request_args['username'] then 
	    if BTWAF_RULES.password[request_args['password']]~=nil then 
	        if request_args['username']=="admin" or request_args['username']=="test" or request_args['username']=="ceshi" then 
    	        ngx.ctx.is_type="弱密码拦截"
				ngx.var.waf2monitor_blocked="弱密码拦截"
                IpInfo.write_log("sql","拦截弱密码"..BTWAF_RULES.password[request_args['password']].."， 如需要关闭请在全局设置中关闭弱密码拦截")
